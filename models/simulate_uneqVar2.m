...
% calculating d.
mu_old = 2; sigma_old = 1;
xx = linspace(-7,mu_old*3*sigma_old,100);
p_new = normpdf(xx);
p_new = p_new/sum(p_new);
p_old = normpdf(xx,mu_old,sigma_old);
p_old = p_old/sum(p_old);
p_d = sigma_old* exp(-0.5*(xx.^2 - (xx-mu_old).^2/sigma_old^2));
p_dold = p_d.*p_old;
p_dold = p_dold/sum(p_dold);
p_dnew = p_d.*p_new;
p_dnew = p_dnew/sum(p_dnew);

subplot(2,1,1)
plot(xx,p_dnew,xx,p_dold)
subplot(2,1,2)
plot(xx,p_new, xx, p_old)
