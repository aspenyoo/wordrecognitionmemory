function [pnew, pold, confBounds] = responses_UVSDd( theta, binningfn, nConf, cplot, dplot)
...
if ~(sigma_mc)
    nSamples = 100;
    nSDs = 3; % how far out you wanna bin
...
%     y = linspace(-nSDs.*sigma_mc,nSDs.*sigma_mc,nSamples);
    ...
    d = x+d0;%-log(sigma_old) - 1/2.*( ((x-mu_old).^2)./sigma_old.^2 - x.^2);
    q = abs(d);
    switch binningfn
        case 2 % logarithmic
