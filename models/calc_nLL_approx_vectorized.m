...
switch modelname
    case 'UVSDd' % if uneqVar
        [pnew, pold, confbounds] = responses_UVSDd(theta, binningfn);
        nLL = -sum(log(pnew).*nnew_part) - sum(log(pold).*nold_part);
        
    case 'UVSDx' % if uneqVar
        [pnew, pold, confbounds] = responses_UVSDx(theta, binningfn);
        nLL = -sum(log(pnew).*nnew_part) - sum(log(pold).*nold_part);
        
    case 'DPSD' % dual process signal detection
        [pnew, pold, confbounds] = responses_DPSD(theta, binningfn);
        nLL = -sum(log(pnew).*nnew_part) - sum(log(pold).*nold_part);
        
    otherwise % if FP, FPheurs, or REM
