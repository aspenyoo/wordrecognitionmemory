function [ varargout ] = nLL_approx_vectorized2( modelname, theta, memstrengthfn, nnew_part, nold_part, fixparams, nX, nS, nConf )
...
if strcmp(modelname,'UVSD') % if uneqVar
    ...
    for iX = 1:nX;
        ...
        switch memstrengthfn
            case 1      % p(correct|evidence). [0 0.5]
                q = 1./(1+exp(-q./k))-0.5;
        end
        
        % non-rounded confidence values
        conf = (a/gamma).*(q.^gamma - 1) + b;
